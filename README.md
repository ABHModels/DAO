...
1.  **Atomic Database (`atdb.fits`)**: The model requires an atomic database in FITS format.***HEAsoft*** has contained it but please make sure the `atdb.fits` file is located in the directory specified by the relevant input paramet if you wish to use a specific version, er.
...
**Consistent Energy Bins**: **This is critical.** The energy grid used to generate the Compton scattering file **must** be identical to the energy grid used in the model simulation. We provide a utility in the `get_redistribution/` directory to help you calculate this file with a consistent grid structure. Please ensure the number of energy bins is the same for both the redistribution file and your model run.
